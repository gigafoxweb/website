...
export default class AdminBannedPlayers extends Vue {
  ...
  public gameModesEnumValues = Object.entries(EGameMode).slice(- (Object.keys(EGameMode).length / 2));
  public gameModesInts = this.gameModesEnumValues.map(x => x[1])
  public gameModesTranslated = this.translateGameModes(this.gameModesEnumValues);
  public gameModesModel = [];
  ...
  public translateGameModes(enumValues : [string, string | EGameMode][]) : LocaleMessage[] { 
    
    let translations = [] as LocaleMessage[]
    for (let item in enumValues) {
      translations.push(this.$i18n.t(`gameModes.${enumValues[item][0].toString()}`))
    }
  ...
  private async init() {
    ...
    // need to create - [{text: string, value: number}]
  }
